...
def insert_mangas(connection, mangas):
	...
	add_mangas= "INSERT INTO MangaFox_Mangas (name, link, img, rate, visual, genre, pos, author, artist, info, votes, status, released, type) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"

	args = (name, link, img, float(rate), int(visual), genre, pos, author, artist, info, int(votes), status, int(released), typ)
